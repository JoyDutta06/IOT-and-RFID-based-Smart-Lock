...
void setup() {
  ...
  pinMode(8, OUTPUT);  //Solenoid Lock PIN OUT
  ...
  if (EEPROM.read(1) != 1) {  // Look EEPROM if Master Card defined, EEPROM address 1 holds if defined
    ...
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.println("SMART DOOR LOCK"); //When no Master Card in Your EEROM (LCD Display)
    lcd.setCursor(0, 1);
    lcd.println(" BASED ON RFID"); //Scan any RFID CARD to set Your Master Card in Your EEROM (LCD Display)
    delay(3000);
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.println("BY JOY DUTTA"); //When no Master Card in Your EEROM (LCD Display)
    lcd.setCursor(0, 1);
    lcd.println("ROLL NO- 16/341"); //Scan any RFID CARD to set Your Master Card in Your EEROM (LCD Display)
    delay(3000);
    lcd.clear();
    lcd.setCursor(0, 0);
